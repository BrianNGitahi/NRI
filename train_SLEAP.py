...
parser.add_argument('--batch-size', type=int, default=64, # used to be 128
...
parser.add_argument('--num-atoms', type=int, default=2,
                    help='Number of atoms (nodes) in simulation.')
...
#save these losses
np.save('SLEAP_val_nll_losses' + '.npy', all_val_nll)
np.save('SLEAP_train_nll_losses' + '.npy', all_train_nll)
np.save('SLEAP_val_mse_losses' + '.npy', all_val_mse)
np.save('SLEAP_train_mse_losses' + '.npy', all_train_mse)
...
val_nll = np.load('SLEAP_val_nll_losses.npy')
train_nll = np.load('SLEAP_train_nll_losses.npy')
val_mse = np.load('SLEAP_val_mse_losses.npy')
train_mse = np.load('SLEAP_train_mse_losses.npy')
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Training and Validation Losses Over Epochs (SLEAP)')
plt.legend()
plt.grid(True)
plt.tight_layout()
...
plt.savefig('SLEAP_MSE_loss_curves.png', dpi=300)  # You can also use .pdf, .svg, etc.
plt.show()
